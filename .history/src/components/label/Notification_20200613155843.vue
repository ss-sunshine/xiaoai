    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane :label="`回收站(${total2})`" name="third">
        <div class="backgroud">
          ...
          <el-button type="info" class="btn" @click="clickItem6">清空回收站</el-button>
        </div>
      </el-tab-pane>
    </el-tabs>
  </div>
...
export default {
  ...
  methods: {
    ...
    clickItem2(index, row) {
      let id = row.id;
      this.item1 = this.arr3.filter(row => {
        return row.id === id;
      });
      //   console.log(this.item1);
      this.arr4.push(this.item1[0]);
      //   console.log(this.arr2);
      this.arr3 = this.arr3.filter(row => {
        return row.id !== id;
      });
    },
    clickItem3(index, row) {
      let id = row.id;
      this.item2 = this.arr4.filter(row => {
        return row.id === id;
      });
      //   console.log(this.item1);
      this.arr3.push(this.item2[0]);
      //   console.log(this.arr2);
      this.arr4 = this.arr4.filter(row => {
        return row.id !== id;
      });
    },
    clickItem4() {
      if ((this.arr.length!==0)) {
        this.arr3 = this.arr3.concat(this.arr);
    ...
    clickItem5() {
      if ((this.arr3.length!==0)) {
        this.arr4 = this.arr4.concat(this.arr3);
